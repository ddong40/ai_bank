...
# 스태킹 모델 (LightGBM + Neural Network OOF 결합)
print(f"\n🔥 스태킹 앙상블 시작...")
print(f"   🤖 Neural Network OOF F1: {oof_f1:.4f}")
print(f"   🌟 LightGBM OOF F1: {oof_f1_lgb:.4f}")

# 스태킹 가중치 최적화 (간단한 가중평균)
def optimize_stacking_weights(nn_pred, lgb_pred, true_labels):
    """최적 스태킹 가중치 탐색"""
    best_score = 0
    best_weight = 0.5
    
    for weight in np.arange(0.0, 1.1, 0.1):
        combined_pred = weight * nn_pred + (1 - weight) * lgb_pred
...
# 최적 가중치 탐색
optimal_weight, optimal_score = optimize_stacking_weights(oof_predictions, oof_predictions_lgb, y)

print(f"   🎯 최적 가중치: NN {optimal_weight:.1f} + LGB {1-optimal_weight:.1f}")
print(f"   🏆 스태킹 F1 Score: {optimal_score:.4f}")

# 최종 테스트 예측 (스태킹 적용)
final_test_predictions = optimal_weight * test_predictions_nn + (1 - optimal_weight) * test_predictions_lgb
...
print(f"   🤖 Neural Network OOF F1: {oof_f1:.4f}")
print(f"   🌟 LightGBM OOF F1: {oof_f1_lgb:.4f}")
print(f"   🔥 스태킹 앙상블 F1: {optimal_score:.4f}")
print(f"   🎯 성능 향상: {optimal_score - max(oof_f1, oof_f1_lgb):.4f}")
...
print("주요 개선사항:")
print("✅ QuantileTransformer(normal) 전처리")
print("✅ CLS 토큰 + Attention Pooling")
print("✅ AdamW + CosineDecay + Weight Decay + Gradient Clipping")
print("✅ StratifiedKFold OOF 교차검증")
print("✅ LightGBM + Neural Network 스태킹 앙상블")
print("=" * 60)