...
model = YOLO('c:/Users/ddong40/Documents/카카오톡 받은 파일/best (2).pt')
'c:/Users/ddong40/Documents/카카오톡 받은 파일/best (2).pt'
...
names = {0: '안지혜', 1: '안준영', 2: '배누리', 3: '최진영', 4: '전사영', 
         5: '정기현', 6: '김호정', 7: '김지혜', 8: '김태운', 9: '박현아', 10: '-'}
...
def generate_frames():
    ...
    while True:
        ...
        if results[0].boxes:  # 객체가 탐지된 경우
            ...
            class_name = names.get(class_id, 'Unknown')  # 클래스 이름 매핑

            # 클래스 이름과 신뢰도를 함께 업데이트
...
@app.route('/')
def index():
    return render_template('webcam_face.html')
