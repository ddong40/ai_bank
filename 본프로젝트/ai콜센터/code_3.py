...
def generate_question_for_missing_info(info_type):
    questions = {
        ...
        "name": "네. 예약자분 성함이 어떻게 되세요?",
        "phone_num": "네. 휴대폰 뒷자리가 어떻게 되세요?"
    }
    return questions.get(info_type, "정보를 입력해 주세요.") # info_type이 question에 존재하지 않는 key 일 경우 '정보를 입력해주세요' 

# 사용자 질문과 키워드를 비교해 알맞은 답변 템플릿 선택
def select_response_template(user_question):
    for situation, data in response_templates.items(): #csv파일을 items() 사용하여 각 변수에 키와 값을 반환
        # 각 상황별 키워드가 질문에 포함되는지 확인
        if any(keyword in user_question for keyword in data['keywords']): #유저의 대답의 keyword가 'keywords' 컬럼에 포함되면
            return data['template'] #46번줄에서 언급됨
...
def chat(user_question):
    ...
    if not missing_info and all(reservation_info.values()):
        if count == 0:
            ...
            speak("네, 예약되었습니다. 추가로 필요하신 사항 있으신가요?")
...
while True:
    ...
    if current_info_type:
        ...
        answer = update_reservation_info(current_info_type, user_input)
        if type(answer) == str:
           print("챗봇 응답:", answer)
           speak(answer)
           continue
