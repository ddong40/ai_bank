...
def solution_model():
    ...
    model = tf.keras.models.Sequential([
        tf.keras.layers.Conv1D(32, 5, 1, padding="causal", input_shape=[N_PAST, N_FEATURES],
                               activation='relu'),
    ...
    model.fit(train_set,epochs=1,verbose=1,
              validation_data=valid_set,batch_size=50
        # YOUR CODE HERE
    )
    ...
    # y_true = []
    print("LOSS :",Loss)
    print(np.mean(abs(valid_set.ravel()-y_pred.ravel())))
    # print("y_pred :",y_pred)
        # YOUR CODE HERE
