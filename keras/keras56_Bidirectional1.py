...
model = Sequential()
model.add(Bidirectional(SimpleRNN(units = 10, activation= 'relu'), input_shape=(3, 1)))
# model.add(GRU(units = 10, activation= 'relu', input_shape=(3, 1)))  #행 무시 열 우선 즉 7을 빼준다. #Rnn은 Dense와 바로 연결이 가능하다.  
model.add(Dense(7))
model.add(Dense(1))
