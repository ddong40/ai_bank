"""
Pure Conv1D 기반 다중 클래스 분류 모델 (Dense layer 완전 제거)
- IQR 기반 이상치 클리핑으로 완화 처리
- Stratified Split으로 라벨 균형 유지
- Conv1D(2층) + Sinusoidal Positional Encoding + Multi-Head Attention Transformer + Conv1D 분류 헤드로 21개 클래스 분류
- Dense layer 없이 Conv1D만으로 특징 추출부터 최종 출력까지 처리
- 최종 출력도 Conv1D(kernel_size=1) + GlobalAveragePooling1D + Softmax로 구성
...
def create_dual_branch_model(input_dim, num_classes=21):
    ...
    # ===== Conv1D Classification Head =====
    # Transformer 출력을 Conv1D로 추가 처리하여 시퀀스 정보 더 활용
    # transformer_out shape: (batch, 26, 48)
    
    # 첫 번째 Conv1D: 더 많은 채널로 확장하며 패턴 추출
    clf = Conv1D(128, kernel_size=3, padding='same', activation='relu', name='clf_conv1')(transformer_out)
    clf = LayerNormalization(epsilon=1e-6, name='clf_ln1')(clf)
    clf = Dropout(0.2, name='clf_dropout1')(clf)
    
    # 두 번째 Conv1D: 채널 감소하며 더 고수준 특징 추출
    ...
    clf = Conv1D(num_classes, kernel_size=1, padding='same', name='clf_conv_output')(clf)  # (batch, 26, 21)
...
print(f"   🔧 모델 구조: X 브랜치(Dilated 1,2) + Z 브랜치(Dilated 4,8) + Transformer + Pure Conv1D")
print(f"   🌟 특징: 듀얼 Dilated Conv1D → Positional Encoding → Multi-Head Attention → Pure Conv1D Output")
print(f"   🚫 Dense layer 완전 제거: Conv1D(1x1) + GAP + Softmax로 최종 출력")
print("=" * 60)
...
# 결과 저장
output_path = 'C:/Users/jsy/Desktop/coretech/Dacon/smart/data/pure_conv1d_submission.csv'
...
print("\n" + "=" * 60)
print("Pure Conv1D 분류 모델 (Dense layer 제거) 학습 및 예측 완료!")
print("=" * 60)