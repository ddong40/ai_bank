...
class Custom_Dataset(Dataset):
    def __init__(self):
        ...
        self.y = (self.y - np.min(self.y, axis=0)) / (np.max(self.y, axis=0)  - np.min(self.y, axis=0)) #minmax scaler
...
with torch.no_grad():
    model.load_state_dict(torch.load(save_path+'t26.pth', map_location=DEVICE,
                                     weights_only=True,
                                     ))
